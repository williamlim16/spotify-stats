...
def taskflow_api():
    ...
    def extractStreamingHistory():
        ...
        combined_df.rename(columns={'endTime': 'end_time', 'trackName': 'track_name',
                           'msPlayed': 'ms_played', 'artistName': 'artist_name'}, inplace=True)
    ...
    def extractArtists():
        ...
        for index, row in artists.iterrows():
            ...
            response = requests.get(
                "https://api.spotify.com/v1/search", headers=headers, params=search_params)
            if len(response.json()['artists']['items']) == 0:
                continue
            artists.at[index, 'id'] = response.json()[
                'artists']['items'][0]['id']
            if len(response.json()['artists']['items'][0]['images']) != 0:
                artists.at[index, 'image'] = response.json(
                )['artists']['items'][0]['images'][0]['url']
            artists.at[index, 'url'] = response.json(
            )['artists']['items'][0]['external_urls']['spotify']
            if response.json()['artists']['items'][0]['followers']['total'] is not None:
                artists.at[index, 'followers'] = response.json(
                )['artists']['items'][0]['followers']['total']
            if response.json()['artists']['items'][0]['popularity'] is not None:
                artists.at[index, 'popularity'] = response.json(
